...
status_code str_valid_check(char const* str, long long* number, long long base)
{
...
status_code get_string(char** string)
{
...
status_code get_max_number(long long base, long long* result, char** answer)
{
    int size = 0;
    char* str = NULL;
    char* max_string;
...
status_code remove_zeros(char** s, int length)
{
    int is_host_zero = 1;
    int position = 0;
    int is_negative = 0;
...
status_code convert(long long number, int base, char res[])
{
...
int main()
{
    ...
    if (remove_zeros(&max_str, strlen(max_str)) != OK)
    {
        printf("Bad alloc\n");
        free(max_str);
        return BAD_ALLOC;
